...
$("#addGallery").click(function (e) {
    ...
    formData.append("gtype", $("#gtype").val());
    formData.append("fileurl", $("#fileurl").val());
    formData.append("details", $("#details").val());
    var TotalFiles = $('#timage')[0].files.length; //Total files
    var timage = $('#timage')[0];
    for (var i = 0; i < TotalFiles; i++) {
        formData.append('timage' + i, timage.files[i]);
    }
    formData.append('timage', TotalFiles);
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data) {
                new PNotify({
                    title: "Success!",
                    text: "Gallery Inserted Successfully.",
                    type: "success",
                });
            }
        },
        error: function (error) {
            $("#nameNameError").text(
                ...
                error.responseJSON.errors.gtype
            ); $("#fileurlNameError").text(
                error.responseJSON.errors.fileurl
            ); $("#detailsError").text(
